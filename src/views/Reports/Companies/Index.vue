<template>
  <div>
    <v-app id="inspire">
    <h4 class="sub-heading">Companies</h4>
    <v-card-title>
      <v-spacer></v-spacer>
      <v-text-field
          v-model="search"
      ...
      <div class="form-group">
        <download-excel
            :data ="getCompaniesFormatted"
            :title = "'Companies'"
            v-if="getCompaniesFormatted.length"
            name="companies.xls"
  ...
  watch:{
    period(){
      if (this.start_date && this.to_date){
        this.$store.dispatch('setCompanies',{from:this.start_date,to:this.to_date});
      }
    },
  },

  computed:{
    ...mapGetters(['getCompanies','getCompaniesFormatted']),
    period(){
